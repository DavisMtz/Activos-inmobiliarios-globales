...
import type { Pregunta } from "../../../server/db/configuracion";
import type { EntregaPublica } from "../../../server/db/entregas";
import { IconoFlecha, IconoMas } from "./iconos";
...
/**
 * Desde el 19/09/2026 esto son las **entregas**: lo que cuenta el cliente
 * desde su enlace, con su permiso, y la foto del día cuando también la
 * autorizó. El comentario y la foto son independientes porque los dos
 * permisos lo son (`shared/entrega.ts`).
 */
export function Testimonios({ entregas }: { entregas: EntregaPublica[] }) {
  ...
  return (
    <section aria-labelledby="titulo-testimonios" className="mx-auto max-w-sitio px-5 py-16 sm:py-24 lg:px-10">
      ...
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)] lg:gap-16 3xl:gap-24">
        <header data-animar>
          <Antetitulo>Entregas</Antetitulo>
          ...
          <p className="mt-4 max-w-sm text-texto-suave">
            Familias a las que ya les entregamos su casa. Cada una nos dio permiso de compartir lo suyo.
          </p>
          <Link
            to="/entregas"
            viewTransition
        ...
        <ul data-animar-lista className="mt-10 flex flex-col lg:mt-0">
          {entregas.map((entrega) => {
            const foto = entrega.fotos[0] ?? null;
            return (
              <li key={entrega.id} className="border-t border-linea py-10 first:border-t-0 first:pt-0 last:pb-0">
                <figure className={foto ? "gap-8 sm:grid sm:grid-cols-[minmax(0,15rem)_minmax(0,1fr)]" : ""}>
                  {foto ? (
                    <img
                      src={foto.src}
                  ...
                  <div className={foto ? "mt-6 sm:mt-0" : ""}>
                    {entrega.comentario ? (
                      <>
                        ...
                        <blockquote className="mt-3">
                          <p
                            className={`font-display leading-[1.3] font-light whitespace-pre-line text-tinta ${
                              foto
                                ? "text-[clamp(1.2rem,1rem+0.7vw,1.7rem)]"
                                : "text-[clamp(1.35rem,1.05rem+1.2vw,2.1rem)]"
                            }`}
                          >
                            {entrega.comentario}
                          </p>
                    ...
                    <figcaption
                      className={`flex items-center gap-3 text-sm font-bold tracking-wide text-tinta ${
                        entrega.comentario ? "mt-6" : ""
                      }`}
                    >
                      <span aria-hidden="true" className="h-px w-8 shrink-0 bg-marca" />
                      {entrega.nombre}
