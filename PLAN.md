...
1. **Vocabulario** (`shared/frase.ts`, gratis): operación, tipo, orden («barato», «más grande», «de lujo»), recámaras, baños, clave («aig 42») y los rasgos más pedidos («alberca», «una planta», «roof garden»), con los errores de oído y de dedo perdonados por `shared/parecido.ts` (`s/z/c`, `b/v`, `h` muda, `k/c/qu`, letra doblada y trasposición): «kasa en benta 3 rrecamaras» se lee entera sin preguntarle a nadie.
...
3. **El modelo** (`server/ia/motor.ts`), solo si quedó algo sin explicar: precios («que no pase de 15 mil», «entre 3 y 4.5 millones»), lugares dentro de una frase larga y rasgos fuera de lista («vista al lago»). Devuelve **solo** `lugar`, `recamaras`, `banos`, `precio_min`, `precio_max` y `palabras`; **nunca casas**. Operación, tipo y orden **no se le preguntan**: medido, el modelo chico ponía `operacion: "venta"` en 9 de 15 frases que no lo decían. `validarIntencion` (`shared/intencion.ts`) exige que cada dato salga de la frase: el número de recámaras tiene que estar escrito, un precio necesita alguna cantidad, y lugar y rasgos solo pueden salir de palabras que el vocabulario no explicó.
...
**Candados del gasto:** interruptor y **tope diario de consultas** (300 de fábrica) en Panel › Configuración; **memoria** de 7 días por huella de la frase (`ia_cache`: una frase se pregunta una vez); freno de 10 consultas por minuto y por persona (`LIMITE_IA`); **3 s de reloj** (si no contesta, queda lo de las capas 1 y 2); ni robots ni precargas del navegador pagan inferencia; frases de más de 160 caracteres no se interpretan. Los Neurons no se pueden leer desde el Worker: el tope va en consultas y el panel estima los Neurons con los tokens.

**El modelo se eligió midiendo** (`npm run medir:ia`: 51 frases reales × 2 vueltas contra la API, con las mismas instrucciones y el mismo validador que el Worker). Resultados del 20/09/2026 en §19. De fábrica, **`@cf/zai-org/glm-4.7-flash`**; el panel deja cambiar entre los cuatro medidos. Hay dos familias de modelo y el motor habla con las dos (§17).

**Apagado** (o sin presupuesto, o lento), el buscador conserva las capas 1 y 2: sigue entendiendo «casa en renta en altosano con alberca»; deja de entender precios y frases largas.
...
| F5 · Buscador que entiende frases (Workers AI) | 🟡 construido y medido en local | 20/09/2026 | Pedido: que los buscadores usen las Neurons de Cloudflare para entender palabras clave, errores al escribir y lenguaje natural, con interruptor en el panel. Diseño en **§10.5**; trampas en §17. **El modelo se eligió midiendo** (`npm run medir:ia`, 51 frases reales × 2 vueltas contra la API de Workers AI, con las instrucciones y el validador del Worker; «perfecta» = todos los campos bien y ninguno de más): **`llama-3.3-70b-instruct-fp8-fast` 102/102** (p50 1.33 s · p90 1.57 s · ~26.6 Neurons por búsqueda), **`glm-4.7-flash` 100/102** (0.97 s · 1.57 s · ~5.2; una cola de 14 s que el reloj corta), `gemma-4-26b-a4b-it` 97/102 (1.25 s · ~7.2), `llama-3.2-3b-instruct` 86/102 (0.32 s · ~4.3; confunde rasgos con colonias y «más de» con «hasta»), `gpt-oss-20b` 81/102, `granite-4.0-h-micro` 76/102 (el más barato, ~1.35, y el más lento: 1.5 s); `qwen3-30b-a3b-fp8` contesta vacío (razona) y `llama-3.1-8b-fp8` no acepta JSON Schema. **De fábrica, GLM 4.7 Flash**: 5 veces más barato que el 70B y más rápido, con 2 frases de diferencia; ~1,900 búsquedas al día dentro de los 10,000 Neurons incluidos, y eso sin contar que el vocabulario resuelve la mayoría sin preguntar. **Medido en local contra `vite preview` y el binding real:** `npm run verificar:busqueda -- --local --ia` **47/47**, tres corridas seguidas (lo que se entiende sin modelo; lo que no se reinterpreta —`literal`, `frase`, página 2, la API, un texto de 300 caracteres, una frase con `<script>`—; la frase **tecleada en la portada con Chrome y enviada con Intro**, que navega sin recargar; el interruptor **pulsado en el panel** con un director desechable, que apaga y enciende, cambia la portada, queda en la bitácora y no se apaga con un `PATCH` que solo trae el tope; y el modelo de verdad: tope de precio con cifra al azar, rango de renta, memoria —la segunda vez en mayúsculas sale igual y sube `de_cache`—, un robot que no paga, y la memoria sin texto de nadie). Por el binding en local el modelo tardó **1.5 s de media** (17 consultas, 2 cortadas por el reloj de 3 s; en local el binding va y vuelve por la red de esta máquina). Vitest **325/325** (58 nuevas: parecido, frase, intención, lugares, rasgos), `tsc -b` limpio, y todo lo anterior sin cambios: `verificar:f2` 39/39, `listado` 24/24, `vitrina` 22/22, `movimiento` 0 invisibles, `f3` 30/30, `f3-navegador` 15/15, `f4` 42/42, `entregas` 33/33. El paquete público no cargó con el vocabulario (trozo de filtros: 641 B gzip, igual que antes). **Migración `0005`: va PRIMERO al desplegar.** |
