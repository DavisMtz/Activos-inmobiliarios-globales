...
- **Antes de correrla, decidir con el usuario el gasto (medido en F1).** Son 3,241 fotos y ~450 MB. Guardarlas cuesta poco (~0.5 créditos al mes), pero **cada variante que se pide por primera vez es una transformación**: con las de §13.3 (galería en 3 anchos, miniatura, tarjeta en 3 anchos y `og`) y `f_auto` generando un formato por navegador, recorrer el catálogo entero puede sumar decenas de miles de transformaciones (del orden de 15 a 30 créditos). El plan gratuito da **25 créditos al mes** y esa nube la comparten Cuponera, «un» y la Biblioteca. Estimación, no medición: hay que confirmarla con `GET /usage` tras `--limite 20`. Opciones: menos anchos en `srcset`, `f_webp` en vez de `f_auto`, o dejar la demo con el puente de WordPress.
