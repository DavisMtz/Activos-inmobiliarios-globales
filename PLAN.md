...
| **Al agrupar `eventos` por casa, SQLite prefería recorrer ENTERO el índice `idx_eventos` antes que usar la ventana de fechas**: `EXPLAIN QUERY PLAN` de `SELECT propiedad_id, tipo, COUNT(*) … WHERE creado_en >= ? GROUP BY propiedad_id, tipo` dio `SCAN e USING COVERING INDEX idx_eventos`, o sea todas las visitas de la historia en cada carga de Métricas, aunque se pidieran 7 días | Con ventana, `FROM eventos e INDEXED BY idx_eventos_fecha` (`server/db/panel/metricas.ts`); sin ventana («Todo») NO se pone, porque sin condición sobre `creado_en` el `INDEXED BY` hace fallar la consulta entera en vez de recorrer. Verlo con `EXPLAIN QUERY PLAN` en la D1 local, que es gratis | medido el 21/09/2026 (tablero de Métricas) |
| **`substr(creado_en, 1, 10)` parte el día a las 18:00 de Morelia**: las fechas de la base van en UTC, y una visita de las 7 de la tarde contaría para mañana | En SQL, `date(creado_en, '-6 hours')` (SQLite lee el ISO con `T` y `Z`: `2026-09-17T05:26:09.107Z` → `2026-09-16`, `…T06:00:00.000Z` → `2026-09-17`, medido en local); en JavaScript, `diaDeMorelia` de `server/fechas.ts`, que da lo mismo. Hora y día de la semana: `strftime('%H' / '%w', creado_en, '-6 hours')`, con `%w` = 0 el domingo | idem |
| **Git Bash también se come una barra de los argumentos que empiezan por `//`**: al empalmar un archivo buscando el marcador `// ─── Barras…`, el guion recibió `/ ─── Barras…`, cortó a media pareja de barras y dejó un `/` suelto que tiró el `.tsx` («Unterminated regular expression literal») | Anteponer `MSYS_NO_PATHCONV=1`, o pasar el marcador dentro de un archivo o del propio guion. Es la misma trampa de las rutas que empiezan por `/` (arriba), con otra cara | idem |
| **El criterio 8 solo vigila los trozos que contienen sus frases-marca** (`MARCAS` en `verificar-f3-navegador.mjs`: «Pegar texto de Facebook», «Hacer portada», «Mandar a la papelera», «Contraseña temporal»). El trozo de Métricas —10 KB gzip con las gráficas— no traía ninguna, y la comprobación dio 17/17 en producción sin haberlo mirado | Cada pantalla nueva del panel pone una frase suya en `MARCAS` (Métricas: «Qué busca la gente») y se comprueba que la lista de «trozos identificados» creció. Para verlo sin navegador: buscar las frases en `build/client/assets/*.js` | medido al desplegar Métricas (22/09/2026) |
...
| F5 · Buscador que entiende frases (Workers AI) | ✅ EN PRODUCCIÓN | 20/09/2026 | **Desplegado a pedido del usuario («Despliega»): Worker versión `0791a369`** (la anterior, para revertir, `0ed0ddcf`), con la migración `0005` aplicada ANTES y tras revisar worktrees, ramas y versión activa. **Medido en producción:** `verificar:busqueda -- --remote --ia` **56/56** (con el modelo real: «casa en altosano con cava de vinos de menos de 3 millones 376 mil» → casas, «Altozano», hasta $3,376,000 y el rasgo que puso el modelo; la segunda vez salió de la memoria; la consulta tardó **1.6 s** por el binding directo), `verificar:f2` **39/39**, `verificar:listado` **24/24**, `verificar:movimiento` 0 invisibles y `verificar:f3-navegador` **17/17** (criterio 8: 113 peticiones en 4 páginas, ninguna del panel). **Cerrado mirando la base y la nube:** 188 casas, ninguna de prueba, 6 servicios, el ajuste `busqueda_ia` como estaba (encendido), `ia_uso` con 1 consulta y 1 de memoria, Cloudinary en ceros (3,241 / 3,241), y las mismas 2 cuentas `@ejemplo.invalid` ajenas de antes (`verificar:entregas` del 19/09), ninguna nueva. **Peso público:** `/propiedades` descarga 125.2 KB gzip de JavaScript contra el tope de 150 (el trozo del listado, con «Así lo entendimos» dentro, pesa 5.9 KB; el de filtros, 642 B, igual que antes). Pedido: que los buscadores usen las Neurons de Cloudflare para entender palabras clave, errores al escribir y lenguaje natural, con interruptor en el panel. Diseño en **§10.5**; trampas en §17. **Tres capas:** vocabulario y precios por código, el catálogo para el lugar, y el modelo solo para lo que sobra; lo entendido acaba siempre en la URL (redirección), así que la página 2, el scroll, la API y un enlace compartido no dependen de la IA. **El modelo se eligió midiendo** (`npm run medir:ia`, 65 frases reales × 2 vueltas contra la API de Workers AI, con las instrucciones y el validador del Worker; «perfecta» = todos los campos bien y ninguno de más): **`granite-4.0-h-micro` 128/130** (p50 1.29 s · p90 1.65 s · máx 3.0 s · **~1.1 Neurons** por búsqueda ≈ 9,400 búsquedas dentro de los 10,000 Neurons diarios), **`llama-3.3-70b-instruct-fp8-fast` 128/130** (1.13 s · 1.51 s · ~19.6 Neurons), `gemma-4-26b-a4b-it` 124/130 (1.20 s · ~4.9), `glm-4.7-flash` 123/130 (0.87 s de mediana, pero **5 de 130 consultas colgadas más de 10 s**) y `llama-3.2-3b-instruct` 117/130 (0.38 s; toma por colonia palabras que no lo son). `qwen3-30b-a3b-fp8` contesta vacío (razona) y `llama-3.1-8b-fp8` no acepta JSON Schema. **De fábrica, Granite Micro.** La primera medición, con el modelo cargando también con operación, tipo y precios, había dado lo contrario (Granite 76/102, GLM 100/102): achicar el trabajo cambió al ganador (§17). **Sin modelo se resuelven 51 de las 65 frases**; solo 14 (rasgos fuera de lista) llegan a preguntar. **Medido en local contra `vite preview` y el binding real:** `npm run verificar:busqueda -- --local --ia` **56/56**, tres corridas seguidas: lo que se entiende sin modelo (incluidos los precios: «menos de 2 millones 251 mil» → $2,251,000, rangos, mínimos, y que los metros no son dinero), que la frase manda sobre lo que quedó en el formulario, lo que no se reinterpreta (`literal`, `frase`, página 2, la API, un texto de 300 caracteres, una frase con `<script>`), la frase **tecleada en la portada con Chrome y enviada con Intro** (navega sin recargar), quitar filtros con un toque, una **segunda frase tecleada en el listado** (un solo formulario y los selectores al día), el interruptor **pulsado en el panel** con un director desechable (apaga y enciende, cambia la portada, queda en la bitácora, y un `PATCH` que solo trae el tope no lo apaga) y el modelo de verdad (un rasgo fuera de lista con cifra al azar, la memoria —la segunda vez en mayúsculas sale igual y sube `de_cache`—, un robot que no gasta, y la memoria sin texto de nadie). Vitest **341/341** (74 nuevas: parecido, frase, dinero, intención, lugares, rasgos), `tsc -b` limpio, y todo lo anterior sin cambios: `verificar:f2` 39/39, `listado` 24/24, `vitrina` 22/22, `movimiento` 0 invisibles, `f3` 30/30, `f3-navegador` 15/15, `f4` 42/42, `entregas` 33/33. El paquete público no cargó con el vocabulario (trozo de filtros: 641 B gzip, igual que antes). De paso se arregló un defecto que venía de F2: los selectores del listado no reflejaban la dirección tras navegar sin recargar. **Pendiente:** probarlo en el Safari del iPad, y en F6 que la etiqueta `canonical` no lleve `frase` ni `literal`. |
| F5 · El destello del buscador | ✅ EN PRODUCCIÓN | 20/09/2026 | **Desplegado a pedido del usuario («Despliegas a produccion»): Worker versión `18878e94`** (la anterior, para revertir, `0791a369`); sin migración, y antes se revisaron worktrees, ramas y versión activa (era la esperada). **Pedido:** que «se usó la IA» deje de decirse con una nota DEBAJO del buscador y lo diga el propio buscador, y que ARRIBA se vean los filtros que se aplicaron. **Hecho:** `app/components/publico/marco-estelar.tsx`, el `StarBorder` de React Bits (variante JS + CSS, **sin dependencias**; copiado a mano, el registro `npx shadcn` no aplica aquí) adaptado: envuelve un panel en vez de ser un botón negro, el destello es `--color-marca` sobre un halo `--color-marca-suave` de 2 px (nació en 3 y él lo pidió más fino), y la sombra pasa al marco de afuera, porque el `overflow: hidden` que necesita la recortaba. Se enciende mientras entiende y **sigue encendido mientras lo entendido siga puesto**; en la portada solo mientras entiende, porque la respuesta se ve en la página siguiente. Todo el movimiento es CSS (`app.css`): con «menos movimiento» las manchas no se dibujan y queda el halo quieto, para que la señal no se pierda. «Así lo entendimos» subió ARRIBA del formulario y compactó su rótulo y la frase en un solo renglón. **Medido en local:** `verificar:busqueda --local` 50/50, `verificar:f2` 39/39, `verificar:listado` 24/24, `verificar:movimiento` 0 invisibles, Vitest 341/341, `tsc -b` limpio. **Medido en producción:** `verificar:busqueda --remote` **50/50** y `verificar:f2 --remote` **39/39**. **Peso:** el trozo nuevo son 0.30 KB gzip y el CSS subió ~0.3 KB. **Trampa nueva:** aquí **no hay configuración de Prettier**, así que `npx prettier --write` reformatea a 80 columnas y deja un diff enorme de ruido: no se corre sobre archivos del proyecto. |
| F5 · Escenario de la portada | ✅ EN PRODUCCIÓN | 20/09/2026 | **Desplegado dos veces a pedido del usuario: `bd0fe253` («despliegas») y, con el retoque de color y tipografía, `20cd3c29` («Despliega»)** (la anterior, para revertir, `18878e94`), sin migración y tras revisar worktrees, ramas y versión activa. **Medido en producción:** `verificar:f2 --remote` **39/39**, `verificar:vitrina` **22/22** y `verificar:busqueda --remote` **50/50**. **Pedido:** que la portada abra con el efecto `ScrollExpand` de React Bits, con la foto que se configure en el panel de fondo (y si no, la de la casa más reciente), el titular centrado, y el carrusel y el buscador debajo. **Hecho:** `app/components/publico/escenario-portada.tsx`, copiado a mano y adaptado (sin dependencias). La foto es `vitrina[0].fotoGrande`, o sea la **casa elegida en Panel › Contenido** o la más reciente, en la variante `galeria` de 1600 px que ya existía (no es un derivado nuevo, §13.5): **no hizo falta tocar el panel ni la base**. El titular, el saludo y el lema se mudaron al escenario; abajo quedaron el buscador (con un rótulo nuevo, «Encuentra tu propiedad», porque sin titular la columna empezaba en una tarjeta suelta) y la vitrina, intactos. **Cuatro adaptaciones que hubo que medir:** (1) el original se queda en cero sin JavaScript, porque el alto de la escena y del riel se los pone el guión: ahora los trae el CSS y el guión solo los afina (comprobado con `setScriptExecutionDisabled`: escena de 900 px y `<h1>` en su sitio); (2) su velo va de 0 a 0.45 y **aquí va al revés**, entero al principio y al 45 % al abrirse, porque sobre la fachada blanca de la casa de portada el titular blanco no se leía; (3) el encuadre del 42 % en un teléfono de 390 px es una estampilla de 164 px, así que se mide por ancho (86 / 64 / 42 %); (4) la escena empezaba debajo de la cabecera y su pie —la seña de «baja»— caía fuera de la primera pantalla: sube 4.75rem y la píldora del menú flota sobre la foto. **Dos guiones se arreglaron, no el diseño:** `verificar:f2` marcaba la foto ampliada como «se sale del ancho» cuando su escena la RECORTA (ahora exime también `overflow-x: hidden`; el desbordamiento de verdad lo sigue cazando el `scrollWidth`), y `verificar:vitrina` y `verificar:movimiento` veían la vitrina quieta porque **se detiene fuera de la pantalla a propósito** y ahora nace dos pantallas abajo: la llevan a la vista antes de mirarla. **Medido contra `vite preview`:** `verificar:f2` **39/39**, `verificar:vitrina` **22/22**, `verificar:movimiento` 0 invisibles en las dos rutas, `verificar:busqueda --local` 50/50, `verificar:listado` 24/24, Vitest 341/341 y `tsc -b` limpio. **Peso:** el trozo de la portada pasó de 7.19 a 8.23 KB gzip y el CSS, de 13.25 a 13.61. **Retoque del mismo día, tras verlo en pantalla («el color negro no queda»):** el campo pasa de tinta a **vino** con degradado radial (aquí el vino se usa como CAMPO grande, no como borde), el velo de la foto se entona a vino y la seña de «baja» toma `--color-sobre-vino-suave`, porque el rojo de la marca sobre vino no contrasta. El titular estrena **Cormorant Garamond** a `clamp(2.9rem, 1.9rem + 4.6vw, 6rem)` (antes 4.25rem en Jost); se importa SOLO en `inicio.tsx` y se precarga desde ahí, así que las demás páginas no bajan sus 37 KB. **A 8rem tapaba la foto entera** y las palabras rozaban las dos orillas: 6rem es donde manda el titular y la casa se sigue viendo. Revalidado en local: f2 39/39, vitrina 22/22, movimiento 0 invisibles, Vitest 341/341. **EN PRODUCCIÓN a pedido suyo («Despliega»): Worker `20cd3c29`** (para revertir, `bd0fe253`); medido ahí: f2 **39/39**, vitrina **22/22** y búsqueda **50/50**. **Y el vino tampoco convenció** («ni la fuente ni el color de fondo»): en vez de adivinar por tercera vez se le enseñaron **cuatro propuestas capturadas sobre la página real** (crema+Playfair, café oscuro+Instrument Serif, vino profundo+Archivo, crema+Jost ligera) y eligió la primera. **Definitivo, en producción («y despliegas»): Worker `ecb59d6c`** (para revertir, `20cd3c29`). Campo **crema** (el papel de la casa: en claro, lo único saturado de la primera pantalla es la casa), el velo de la foto **aclara en vez de oscurecer** porque el titular es de tinta, y **Playfair Display** en lugar de Cormorant, con los colores de siempre (rojo de marca en el filete, gris en el lema). **La cuesta de la escala la fija el teléfono:** a 390 px «Comercialización,» es la palabra más ancha y con 48 px dejaba **4 px** de aire a cada lado (medido en seis anchos: 390, 430, 768, 1024, 1440 y 1920); con `clamp(2.4rem, 1.46rem + 5.05vw, 6rem)` quedan 20 px y el escritorio se queda igual en 96 px. Medido en producción: f2 **39/39** y vitrina **22/22** (la primera corrida dio 21/22 y la repetición 22/22: tiempos contra la red, no el sitio). **Técnica que vale la pena repetir:** las propuestas se capturaron inyectando CSS con CDP sobre la página ya compilada y bajando las fuentes candidatas de Google Fonts, sin instalar ni recompilar nada; solo la elegida se instaló (`@fontsource-variable/playfair-display`). **Ojo:** el LCP de la portada ya no es el titular sino la foto de 1600 px, y F2 venía con Lighthouse móvil 82 contra la meta de 90: hay que volver a medirlo en producción. |
| F5 · La portada que vende (composición 2) | ✅ EN PRODUCCIÓN | 20/09/2026 | **Desplegado a pedido del usuario («despliegas»): Worker versión `2168665c`** (para revertir, `ecb59d6c`; antes, `20cd3c29` y `bd0fe253`). **Pedido:** «la portada como que aún no me gusta, quiero que sea atractiva y que se vea profesional enfocada en ventas y con una excelente composición». **Medido ANTES, en producción:** el titular de tinta iba MONTADO sobre la foto y para leerlo había que lavarla; la foto era una estampilla del 42 % en un campo crema (más de media pantalla vacía); en la primera pantalla no había ni buscador ni precio ni WhatsApp —el buscador quedaba a **1 400 px de scroll**, tras un riel de 1.9 pantallas—; y la casa del escenario era la MISMA que la primera de la vitrina. **Cómo se eligió:** cuatro composiciones distintas armadas SOBRE LA PÁGINA REAL y capturadas en 1440×900 y 390×844; eligió la 2 en un mensaje («Me gusto la dos»). La receta de las maquetas, abajo. **Lo que quedó:** la foto llena la primera pantalla y en su pie van el titular, la casa con su precio, las tres cifras y el buscador en un renglón (desde `lg`); la segunda pantalla lleva el rótulo, los accesos por tipo y la vitrina de siempre. **Medido DESPUÉS, contra `vite preview` en 390/768/1024/1280/1366/1920:** el escenario llena la pantalla en los seis, **cero desbordes horizontales**, y el buscador siempre termina dentro (824 de 844 en el teléfono, 1050 de 1080 a 1920). Titular a 390: **32.89 px** y el renglón más ancho **320 px de 350** disponibles. **En producción:** `verificar:f2 --remote` **39/39** y `verificar:vitrina` **22/22**; en local, `verificar:movimiento` 29 animados y **0 invisibles** (también con «menos movimiento») y `verificar:navegador` 10/10. **Trampas nuevas:** (1) **un velo por porcentajes no sirve para un pie que cambia de alto** —mide 716 px en un teléfono y 456 en un escritorio ancho—: el titular del teléfono caía en un tramo de 0.34 de tinta y se perdía sobre la fachada blanca (`h1` de 240 a 343 px en una pantalla de 844). El degradado se ató al pie y sus paradas van en **rem** desde arriba, así que el texto va siempre sobre 0.86 o más. (2) **Un héroe de `100vh` colocado DEBAJO de la cabecera mide `100vh` + 76 px** y su degradado de abajo cae fuera de la pantalla; sube con `margin-top: -4.75rem`, como ya hacía el escenario. (3) **El parallax va con `translate`, no con `scale`:** la foto mide 112 % del alto y se mueve como mucho un 5.5 %, así que nunca descubre una orilla. (4) `verificar:vitrina` tabulaba **un paso fijo** desde el botón del buscador hasta la casa; entre los dos hay ahora nueve enlaces más, así que ahora tabula HASTA encontrarla (y su `--base` por omisión es el 4180, no el 5180 del preview: sin pasarlo, da 0 montadas y parece rota). **Decidido con él:** el antetítulo y el lema salen de «Saludo» y «Lema» en Panel › Contenido (hoy vacíos) y la foto de portada se elige en Panel › Contenido —la de hoy, AIG-0188, tiene alambre de púas a la izquierda y una obra de ladrillo a la derecha—. |
| F5 · La composición de la portada, ordenada | ✅ EN PRODUCCIÓN | 20/09/2026 | **Desplegado a pedido del usuario («sube y despliega»), con el `git push` en la misma orden: Worker versión `020cc1b9`** (para revertir, `2168665c`; antes, `ecb59d6c` y `20cd3c29`), **sin migración** y tras revisar worktrees, ramas y `deployments list` (la activa era la esperada). **Verificado EN PRODUCCIÓN:** `verificar:f2 --remote` 39/39, `verificar:vitrina` 22/22, `verificar:movimiento` sin nada invisible, el HTML servido sin JS con AIG-0188 en el héroe y AIG-0184 en la vitrina, y la portada retratada con `desbordeX` 0. **Pedido:** «Revisa la index… ayúdame a hacer que todos los contenidos estén armónicamente ordenados… le falta una última pulida a la composición». Commit `d658f0f`. **Lo que se midió, no se opinó** (guion `mirar-portada.mjs` en el borrador de la sesión: mapa de secciones + retrato de la página entera; y `alinear.mjs`, los dos bordes de la columna contra la vitrina). **Cinco arreglos:** (1) «Encuentra tu propiedad» flotaba —la columna iba centrada contra una vitrina de 600 px y dejaba **260 px de campo vacío arriba y 230 abajo**—; ahora `lg:items-stretch` + `lg:mt-auto` en el enlace, y los accesos por tipo pasan de píldoras a un **índice en renglones** con la cuenta a la derecha, más las **cinco colonias con más casas** (las mismas de `listado.tsx`). Medido: `difArriba` y `difAbajo` = **0 px** a 1024, 1280, 1440 y 1920. (2) El aire de esa sección era el único propio del sitio (40/48 y 64/48 contra `py-14 sm:py-20`): normalizado, los huecos quedan **160/160/160** en escritorio. (3) **La casa del héroe y la primera tarjeta de la vitrina eran la misma**, misma foto y mismo precio a 900 px de distancia: la vitrina arranca en la segunda y la del héroe pasa al final del ciclo (`casasEnVitrina`, rotación en la vista; no se tocó `casasDePortada` ni `repartirPortada`). (4) En el teléfono las tres cifras caían **2+1**, con «precio desde» sola en un tercer renglón: `grid grid-cols-3` hasta `lg`, el pie del héroe mide **38 px menos** (`h1` de 240 → 278) y se ve más casa. (5) Menores: el enlace decía palabra por palabra lo mismo que el botón del buscador una pantalla más arriba (ahora «Ver todo el catálogo»), y el párrafo del cierre partía en «una a / la medida» (`max-w-xl` → `max-w-2xl`). **Verificado con el preview en el 5180:** 341 pruebas, `tsc -b`, `verificar:f2` 39/39, `verificar:vitrina` 22/22, `verificar:listado` 24/24, `verificar:movimiento` sin nada invisible, `desbordeX` 0 en 390/768/1024/1280/1366/1920. **No se tocó el héroe** (velo, foto, `--text-portada`, `pb`) ni el pie de página. **Falta:** que el dueño elija entre las cuatro maquetas de esa columna (`variante-a…d.png`) y el Lighthouse móvil |
| F5 · Redes desde el panel y el pie ordenado | ✅ EN PRODUCCIÓN | 20/09/2026 | **Desplegado a pedido del usuario («Sube a producción»), con el `git push` en la misma orden: Worker versión `d71de42b`** (para revertir, `020cc1b9`), sin migración y tras revisar worktrees, ramas y `deployments list`. **Verificado en producción:** la fila VIEJA de `redes` se sigue leyendo y el pie enseña Facebook e Instagram con sus enlaces reales; `verificar:f2 --remote` 39/39 y `verificar:movimiento` sin nada invisible. **`verificar:redes --remote` NO se corrió**: sobrescribe temporalmente la fila `redes` de producción; preguntar antes. **Pedido:** «actualizar el pie… dame propuestas y añade la opción de añadir más redes como tiktok, instagram, x y más desde el panel». Commit `503e9d5`. **Las redes, SIN MIGRACIÓN:** la tabla `configuracion` guarda JSON, así que la clave `redes` pasó de `{facebook, instagram}` a `{lista:[{red,url}]}` sin tocar el esquema —el mismo truco que `nosotros.valores`—. **La fila de producción es todavía la vieja** (`{"facebook":"…","instagram":"…"}`, leída de D1 el 20/09) y se convierte al vuelo: si solo se leyera `lista`, el pie se habría quedado sin las dos redes del negocio sin que nadie tocara nada. Se moderniza sola al primer guardado. **La regla vive en `shared/redes.ts`** y la usan las dos puntas, leer y guardar: sin renglones vacíos, sin repetidas (manda la de más arriba), solo `https://`, solo claves del catálogo y tope de 8. **El panel:** ocho renglones fijos con desplegable + enlace, como los «valores» de Nosotros, **y funciona sin JavaScript** (un `select` siempre viaja; vaciar el enlace es como se QUITA una red). **Catálogo:** Facebook, Instagram, TikTok, X, YouTube, LinkedIn, Threads y Canal de WhatsApp. **Los iconos se miraron en grande antes de darlos por buenos** y ahí se cayeron dos: el primer Threads salía como un caracol y la primera X como el aspa de «cerrar» —las dos se rehicieron—. **El pie:** cuatro columnas parejas (marca · Sitio · Redes · Escríbenos) en vez de un tercio para la marca y el resto apretado, que dejaba ~300 px de hueco bajo el lema y otro a la derecha; las redes con su NOMBRE junto al icono, y una cuarta columna que es Contacto cuando haya datos y, mientras no, la invitación a escribir por WhatsApp (que el pie no tenía). **Se le enseñaron cuatro pies capturados** y quedó puesta la D. **Verificado:** 353 pruebas (12 nuevas), `tsc`, `verificar:f2` 39/39, `verificar:f3` 30/30, `verificar:movimiento` sin nada invisible y **`npm run verificar:redes` 10/10, que PULSA «Guardar las redes»** y después mira el pie. **Trampa propia:** ese guion daba por red el botón nuevo de WhatsApp (también es un enlace externo del pie) y por eso hay que buscar dentro de la sección «Redes», no en todo el `footer` |
| F5 · Contacto: el mostrador y el formulario que avisa | ✅ EN PRODUCCIÓN | 21/09/2026 | **Desplegado a pedido del usuario («Despliegas»), con el `git push`: Worker versión `4ba9e535`** (para revertir, `d71de42b`), sin migración y tras revisar worktrees, ramas y `deployments list` (la activa era la esperada). Commit `450da0c`. **Pedido:** «micro interacciones y animaciones así como una mejor composición profesional» para Contacto. **Se le enseñaron cuatro composiciones** retratadas sobre la página real (A mostrador · B índice · C carta · D casa, en `Escritorio\portada-propuestas\contacto-21-09\`) y **quedó puesta la A**; contestó «Despliegas» con ella a la vista. **Composición:** la izquierda era titular + botón frente a un formulario de 640 px (~450 px vacíos a 1440) y la lista de datos se pintaba VACÍA (en producción no hay teléfono, correo ni dirección capturados); ahora es un campo de tinta con WhatsApp, su número (`numeroParaLeer`), los datos de la oficina cuando existan (con «Copiar») y el dibujo de asesoría de Servicios recoloreado por variables (`.escena-contacto`; `--color-marca-claro` #F5515F, porque el rojo de marca da 2.3:1 sobre tinta y este 5.6:1). A 1920+ el titular pasa a `text-display` y el dibujo crece: a 2560 el mostrador mide ~1 500 px. **Formulario:** errores junto al campo con la MISMA regla del servidor (`problemasDeProspecto`, `shared/validacion.ts`; `revisarProspecto` devuelve el primero en el orden de `CAMPOS_DE_PROSPECTO`); sin JavaScript el servidor devuelve el error en su campo y lo tecleado. **Regla nueva: sin teléfono y sin correo no se acepta** (el formulario general aceptaba un nombre solo y el asesor no tenía por dónde contestar); el aviso es del PAR y sale debajo de los dos, en el lugar de la nota. **«¿Qué necesitas?»** (opcional) llena los tipos `vender` y `credito` de `prospectos`, que existían desde el esquema y nadie usaba; comprar y rentar anteponen su frase al mensaje (`shared/contacto.ts`). **Movimiento, todo CSS y siempre por algo que hizo la persona** (nada en bucle, así que no hace falta botón de pausa): el hilo rojo bajo el campo con foco (solo en el tramo recto: un `overflow: hidden` para recortarlo a la curva cortaría también el anillo de foco), la palomita que se dibuja, la casilla, las estelas del isotipo en «Enviar» (se estiran al pasar y corren al mandar), los puntos del globo del dibujo escriben UNA vez al pasar por WhatsApp, al entrar al formulario y al recibir el mensaje, y el gracias llega con el globo rojo del asesor y su palomita, el nombre y el teléfono por el que lo buscarán; la hoja encoge a su alto nuevo (FLIP con WAAPI). **Sin botón flotante en `/contacto`** (tapaba el formulario en el teléfono, como en las entregas). **Verificado:** 366 pruebas (13 nuevas), `tsc`, el detector de `impeccable` limpio, **`verificar:contacto` 32/32 en local y 28/28 en producción, que PULSA los botones** (errores en su campo, el camino sin JavaScript, el gracias con la trampa llena y, solo en local, un envío de verdad que queda como `vender` y se borra), `verificar:f2` 39/39 y `verificar:movimiento` con `/contacto` (23 elementos, 0 invisibles en los dos modos), las dos en local y en producción, y **0 prospectos de prueba** en la base de producción. **Tres trampas:** (1) volver a abrir la MISMA dirección por CDP restaura el desplazamiento y la captura sale con el titular bajo la cabecera (`scrollTo(0, 0)` tras hidratar); (2) `scrollIntoView({ block: "nearest" })` no mueve nada mientras la hoja encoge —todavía tapa la pantalla entera y «ya está a la vista»—: se usa `start` con `scroll-mt-24`, o el gracias queda bajo la cabecera en el teléfono; (3) las pruebas de `tests/` no pueden importar `server/` (su `tsconfig` no trae los tipos del Worker): lo compartido va en `shared/` y el camino entero lo prueba el guion con navegador. **Después del despliegue, con los datos de la oficina SEMBRADOS EN LOCAL** (en producción no hay ninguno y esa rama nunca se había pintado) salieron dos defectos: el correo de la oficina se partía a media palabra (una «m» sola en el renglón siguiente a 1440; «activosinmobili / ariosglobales.com» a 390) y el mostrador medía 1 086 px contra 702 del formulario, con **384 px de hoja vacía**. Arreglo: el icono chico junto a la etiqueta y «Copiar» en esa línea (el dato gana todo el ancho), `<wbr>` tras la «@», el dibujo más chico cuando hay datos y el mensaje que crece hasta llenar la hoja (`.campo-crece`). `verificar:contacto` ganó la sección 7 (solo local: siembra, mide y devuelve la fila) y da **39/39**; `f2` 39/39 y `movimiento` limpio. **Ese ajuste queda en un commit LOCAL, sin empujar ni desplegar**, esperando su palabra. **Peso:** el bloque de Contacto suma **~1.2 KB gzip** al CSS de todas las páginas (14.7 → 15.9 KB) |
| F5 · Métricas con gráficas (el tablero) | ✅ EN PRODUCCIÓN | 22/09/2026 | **Desplegado a pedido del usuario («Despliega», tras ver las capturas de la primera versión), con el `git push`: Worker versión `5bdc97d4`** (para revertir, `4ba9e535`), sin migración y tras revisar worktrees, ramas y `deployments list` (la activa era la esperada). Commits `2593612` y `d70f16b`; con ellos subió `62a2db4` (Contacto: los renglones de la oficina). **Medido en producción:** `verificar:f4` 54/54, `f3-navegador` 17/17 —con el criterio 8 vigilando por fin el trozo de Métricas (abajo)—, `f2` 39/39, `contacto` 28/28, `movimiento` sin nada invisible en los dos modos, y cerrado mirando la base (188 casas; 0 casas, prospectos, zonas o cuentas de prueba nuevas; 0 eventos huérfanos) y `fotos:migrar --remote --verificar` en ceros. **Dos hallazgos de ese cierre:** (a) el criterio 8 reconoce los trozos del panel por cuatro frases y el de Métricas no traía ninguna, así que el primer 17/17 NO lo miraba; con «Qué busca la gente» en `MARCAS` salen 4 trozos y ninguno se pide en 120 peticiones del sitio público (§17); (b) las verificaciones con Chrome sin cabeza (`f2`, `movimiento`, `f3-navegador`) dejan una o dos `ficha_vista` por corrida en producción (07:35 y 07:37 UTC del 22/09, `casa-en-el-prado-4`): pasaba desde antes y ahora se ve en las gráficas; ignorar robots y la cookie del panel en `POST /api/eventos` queda como decisión suya. **Pedido:** «que en panel de administración el apartado de métricas tenga gráficos y demás métricas de forma visual para poder ver el desempeño de todo». Hasta hoy eran cinco cifras y dos tablas, y el comentario de la pantalla decía que eran tablas «a propósito»: el panel no cargaba biblioteca de dibujo. **Se siguió sin biblioteca:** gráficas a mano en `app/components/panel/graficas.tsx` (HTML y SVG con medidas en porcentaje), pintadas en el servidor, cada una con su tabla plegada («Ver los números») y su `title`; el JavaScript solo agrega el globo con las cifras (ratón, dedo y flechas del teclado, con `aria-live`). Color con el método del skill `dataviz` y su `validate_palette.js` contra el blanco de las tarjetas: **tinta = lo que se ve, rojo de marca = lo que se convierte en contacto, gris = contexto**; la rampa roja de los estados (`#e38d98 → #6b0413`) pasa como ordinal (el claro da 2.46:1) y la de tinta del mapa de horas es secuencial. **La pantalla:** (1) cuatro tarjetas —fichas vistas, casas que se vieron (de las del sitio, con medidor), clics en WhatsApp («1 de cada 28 vistas») y prospectos («6 sin atender»)— con su cambio contra el periodo anterior y su tendencia; **con menos de 10 en el periodo anterior el cambio se dice en piezas** («4 menos que…»), no en porcentaje; (2) **día a día**: vistas en área de tinta arriba y contactos en columnas rojas abajo, un eje cada una (dos ejes en una gráfica inventan relaciones), un solo globo para las dos; «Todo» pasa a semanas (>92 días) o meses (>2 años), y **el último punto va punteado y se llama «Hoy», «Esta semana» o «Este mes»**, porque siempre va a medias (entero, la última semana se desplomaba como si el sitio se hubiera caído); (3) **cómo van los prospectos**: barra partida por estado actual de los que llegaron en el periodo, **tiempo para atenderlos** (mediana de horas hasta el primer cambio de estado o la primera nota, de la bitácora) y abiertos sin asignar, con enlace a la bandeja filtrada; (4) las 25 casas con su barra, **10 a la vista y 15 plegadas**; (5) **qué busca la gente**: por tipo, precio de venta y colonia, la parte de las vistas contra la parte del catálogo en el sitio, con «Se busca más de lo que hay» solo con ≥30 vistas en el periodo y ≥5 en el grupo; se nombran 4 tipos y 6 colonias y el resto va en una línea de texto (en barra, «Otras 116 colonias» era la más larga y se comía a las demás); (6) **a qué hora miran**: 7 × 24 en hora de Morelia, con la hora y el día más fuertes dichos en palabras; (7) **el inventario** en una barra partida y lo que se publicó, apartó, vendió o rentó en el periodo (de la bitácora: las casas que llegaron de WordPress no cuentan); (8) por asesor, con barra y «pendientes hoy». **Por rol, en el SQL:** el asesor ve todo eso de SUS casas; contenido, solo vistas (sin panorama de prospectos, sin contactos, sin asesores). **Teléfono y «compartir» ya no salen en cero para siempre:** ninguna página emite `telefono_click` ni `compartir` (medido: `propiedad.tsx` solo avisa vista y WhatsApp; los `tel:` de `marco.tsx` y `contacto.tsx` no avisan) y sus tarjetas salen solo si hay alguno en el periodo. **Datos:** todo en un `db.batch` (más la tabla por asesor), `COUNT` por día, por casa y tipo, por hora, y la cuenta por casa se arma en JavaScript sobre el catálogo (188 filas): así salen la demanda, la cobertura y el inventario sin volver a recorrer `eventos`. **La ventana cambió:** «30 días» son hoy y los 29 de antes en días de Morelia (`server/metricas.ts`, `ventanaDeMetricas`), no 30 × 24 h desde este minuto; el periodo anterior mide lo mismo y **se corta a la misma hora de hoy** (si no, cada mañana todo parecería ir a la baja). **Medido:** 382 pruebas (16 nuevas en `tests/metricas.test.ts`), `tsc` limpio; **`verificar:f4` 54/54** (12 nuevas: la sección 4b compara cada gráfica contra un `SELECT` a mano —serie = total, mapa de horas = vistas, periodo anterior, demanda, inventario, estados, contenido y asesor— y la 5 pide el SVG en el HTML servido) **con la base limpia Y con cuatro meses de muestra**; `verificar:f3` 30/30, `f3-navegador` 15/15, `f2` 39/39 y `busqueda` 50/50 (sin `--ia`), todo contra `vite preview`; en Chrome, el globo de la serie y el del mapa, las flechas del teclado y **cero errores de consola** (sin diferencias de hidratación: los meses y días se escriben con listas propias, no con `Intl`, que en el servidor y en el navegador abrevia distinto). **Trampas:** (1) **la casa de prueba de F4 se quedaba fuera de las 25** en cuanto hay tráfico de verdad (5 vistas y, al empate, gana el id más bajo): ahora recibe más vistas que la 25.ª de toda la historia; en producción esa comprobación se habría roto sola con el tiempo; (2) el `COUNT(*)` a mano de F4 usaba 30 × 24 h y dejaba de cuadrar con la ventana nueva: ahora importa `ventanaDeMetricas` con `importarTs`; (3) las tres de §17 (`INDEXED BY`, el día de Morelia en SQL y la barra que se come Git Bash). **Datos de muestra, SOLO en local:** `npm run metricas:muestra -- --local` siembra 3 791 eventos con ritmo (noche y fin de semana), 74 prospectos con su bitácora, dos asesores y una cuenta de contenido de prueba, deja sesiones y anota en `%TEMP%\aig-metricas-demo.json` lo que tocó; `--limpiar` lo devuelve TODO (medido: 0 cuentas, 0 prospectos, 77 eventos, 188 publicadas, 0 con asesor). Se niega a correr con `--remote`. **Peso:** el JavaScript público no cambia ni un byte; **el CSS común sube 0.65 KB gzip (15.9 → 16.6 KB)** por las utilidades nuevas del panel —el CSS es uno solo para sitio y panel—; el trozo de Métricas del panel pasa de 2.2 a 10.5 KB gzip. Capturas (muestra inventada, local) en `Escritorio\portada-propuestas\metricas-21-09\` |
| F6 · Producción | ⏸ espera consultores | | |

**Cómo se enseñan cuatro composiciones sin tocar el repo (20/09/2026).** Lo que funcionó con
este usuario —y ya había funcionado con el color y la tipografía— es **enseñar, no describir**.
Las cuatro maquetas se armaron sobre la página compilada, no de cero:

1. `curl` de la portada de producción a un archivo.
2. Se le quitan los `<script>` y se le añade `<base href>` al dominio: queda estática (React no
   hidrata) pero con el CSS, las fuentes y las fotos de verdad.
3. Un guion por variante reacomoda ESE DOM con CDP —mueve el panel del buscador, las cifras y
   la tarjeta de la vitrina— y le añade su CSS.
4. Captura a 1440×900 y 390×844, y las ocho a una hoja HTML con el «pero» honesto de cada una.

Trampas de la receta: apagar las animaciones con CSS **congela la cortina de bienvenida** (hay
que quitarles la clase a los nodos, no ponerles `animation: none`); un `transform: none
!important` descentra lo que uno mismo coloca con `translateX`; y mover un nodo reinicia su
`animate-entrada`, así que el bloque del precio salía invisible en la captura.

**Cómo se retrata la portada ENTERA sin que la captura mienta (20/09/2026).** Tres trampas, las
tres vistas en esta sesión, y las tres hacen que la captura parezca un fallo del sitio:

1. **Agrandar la ventana al alto de la página infla el héroe.** Mide `100svh`: con una ventana
   de 4 000 px de alto sale un héroe de 4 000 y la página retratada mide 7 410 px en vez de
   4 155. La ventana se deja en 1440×900 y se recorta más allá con
   `Page.captureScreenshot({ captureBeyondViewport: true, clip: {...} })`.
2. **`transform: none !important` devuelve a su sitio el botón de WhatsApp**, que en el héroe se
   aparta (`in-data-[heroe=dentro]`), y la captura lo enseña encimado al botón de buscar. Para
   dejar la página en reposo basta con `animation-duration: 0.001ms` y `animation-delay: 0`:
   las entradas acaban en su estado final y nada más se mueve.
3. **La foto del héroe se rasteriza en blanco** en un recorte largo, porque vive en su propia
   capa (`will-change: transform`): sale el campo de tinta del reposo —`rgb(17,17,17)` limpio,
   comprobable con un pixel— y parece que la foto no carga. Antes de disparar: `willChange =
   'auto'`, esperar a `img.decode()` y medio segundo.

Y para que las fotos de abajo existan, hay que **recorrer la página con `scrollTo` hasta el
fondo y volver a 0** antes de capturar: son perezosas y salen como campos rosas.
